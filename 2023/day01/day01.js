const data = require("./day01Puzzle");
...
const OVERLAP_REGEX = /oneight|threeight|fiveight|nineight|twone|sevenine|eightwo/g;

const NUM_REGEX = /one|two|three|four|five|six|seven|eight|nine/g;
...
function decode(arr) {
	...
	for (const str of arr) {
		...
		if (newStr.length > 2) {
			let twoNumStr = newStr[0] + newStr[newStr.length - 1];
			nums.push(parseInt(twoNumStr));
			continue;
		}
	}
...
function convertStrNums(str) {
	let overlapStr = str.replace(OVERLAP_REGEX, (match) => overlap[match]);
	let newStr = overlapStr.replace(NUM_REGEX, (match) => numValues[match]);
	return newStr;
}
...
function convertArr(arr) {
	...
	for (const val of arr) {
		output.push(convertStrNums(val));
	}
	return output;
}

let convertedPuzzleArr = convertArr(data.puzzle);
console.log(decode(data.puzzle));
console.log(decode(convertedPuzzleArr));
// console.log(decode(convertArr(puzzle2)));
// console.log(decode(convertArr(example2)));
// console.log(convertArr(puzzle));
console.log(convertStrNums("oneight"));
